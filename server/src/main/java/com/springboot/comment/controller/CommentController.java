...
public class CommentController {
    ...
    public ResponseEntity updateComment (@PathVariable("comment-id") @Positive long commentId,
        ...
        return new ResponseEntity(new SingleResponseDto<>(commentMapper.commentToListResponseDto(comment)), HttpStatus.OK);
    }
    @GetMapping("/comments")
    public ResponseEntity getComments(@Positive @RequestParam int page,
                                      @Positive @RequestParam int size,
                                      @Positive @RequestParam(name = "user-id") long userId,
                                      Authentication authentication) {
        Page<Comment> pageComments = commentService.findComments(page - 1, size, userId, authentication);

        List<Comment> comments = pageComments.getContent();

        return new ResponseEntity(new MultiResponseDto<>(commentMapper.commentsToListResponseDtos(comments), pageComments),HttpStatus.OK);
    }
